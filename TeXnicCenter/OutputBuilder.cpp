                            ...
                            int nPriority /*= THREAD_PRIORITY_BELOW_NORMAL*/)
{
	...
	{
		CString strMsg;
		strMsg.Format(STE_LATEX_START, static_cast<LPCTSTR>(CProfileMap::GetInstance()->GetActiveProfileKey()));
...
BOOL COutputBuilder::RunLatex()
{
	...
	if (!p)
	{
		...
		m_strLatexResult.Format(STE_LATEX_START_FAILED_EXT, static_cast<LPCTSTR>(pc.GetExecutable()), systemError);
		return FALSE;
	}
