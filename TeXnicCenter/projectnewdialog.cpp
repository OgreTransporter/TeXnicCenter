...
BOOL CWizardBasedProjectTemplateItem::InitItem(LPCTSTR lpszPath, CImageList &ImageList32, CImageList &ImageList16)
{
	...
	CString strProgId;
	strProgId.Format(_T("%s.ProjectWizard"), static_cast<LPCTSTR>(CPathTool::GetFileTitle(m_strPath)));
...
void CProjectNewDialog::Create()
{
	...
	if (!CreateDirectoryRecursive(pProject->GetWorkingDirectory()))
	{
		...
		CString errMsg;
		errMsg.Format(STE_PROJECTDIR_CREATE_ERR, static_cast<LPCTSTR>(pProject->GetWorkingDirectory()), systemError);
