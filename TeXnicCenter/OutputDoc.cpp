...
void COutputDoc::OnLatexView()
{
	...
	if (pCmd && pCmd->GetActiveCommand() == CProfile::CCommand::typeProcess)
	{
		...
		{
			CString strFormat;
			strFormat.Format(STE_LATEX_START_FAILED, static_cast<LPCTSTR>(pCmd->GetProcessCommand().GetExecutable()));
	...
	else if (pCmd && pCmd->GetActiveCommand() == CProfile::CCommand::typeDde)
	{
		...
		{
			CString strFormat;
			strFormat.Format(STE_LATEX_START_FAILED, static_cast<LPCTSTR>(pCmd->GetDdeCommand().GetCommand()));
			AfxMessageBox(strFormat,MB_ICONSTOP | MB_OK);
		}
	}
}
...
void COutputDoc::OnEditFindInFiles()
{
	...
	strStart.Format(STE_GREP_START, static_cast<LPCTSTR>(dlg.m_strSearchFor));
