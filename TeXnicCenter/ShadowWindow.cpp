...
void CodeView::ShadowWindow::UpdateStatusBarMessage()
{
	...
	if (!found)
		message.Format(IDS_FIND_INCREMENTAL_FAILED, static_cast<LPCTSTR>(search_text));
	else
		message.Format(search_forward ? IDS_FIND_INCREMENTAL_FORWARD : 
			IDS_FIND_INCREMENTAL_BACKWARD, static_cast<LPCTSTR>(search_text));
		...
		if (rounds > 0) {
			...
			if (search_forward && match_pos >= original_pos || 
				!search_forward && match_pos <= original_pos) {					
					...
					if (found) {
						CString message;
						message.Format(IDS_INCREMENTAL_SEARCH_PASSED_START_SEARCH, static_cast<LPCTSTR>(search_text));
